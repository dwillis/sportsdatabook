...
That presents it's own problems. So let's get set up. The dataset we'll use is college football games between 2011 and 2020.
...
dllink <- download_link(
  link = "https://dwillis.github.io/sportsdatafiles/footballlogs1120.csv",
...
How does this apply in the real world? Let's pretend for a minute that you are Mike Locksley, and you have a mess on your hands. Your job is to win conference titles. To do that, we need to know what attributes of a team we should emphasize. We can do that by looking at what previous Big Ten conference champions looked like.
...
How does that compare to Maryland for this season? 

```{r}
logs %>% 
  filter(
    Team == "Maryland" & Season == 2020
    ) %>% 
  summarise(
    meanNetYards = mean(NetYards),
    meanTurnoverMargin = mean(TurnoverMargin),
...
```{r}
(0.0547465*-20.8) + (3.8806793*-1.4) + (-3.9374905*5.42) + (3.9152803*6.44) + 0.4960303
```

By this model, it predicted we would average being outscored by our opponents by 2.2 points over the season. Reality? We were outscored by 8.4 on average.