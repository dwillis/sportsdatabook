...
dllink <- download_link(
  link = "https://dwillis.github.io/sportsdatafiles/footballlogs20.csv",
...
```
So looking at this table, what you see here are the teams who scored more than their net yards would indicate. Several are pretty eye-opening, including Wisconsin-Wake Forest.

What about Maryland?

```{r}
residualmodel %>% arrange(desc(residuals)) %>% select(Team, Opponent, Result, differential, NetYards, predicted, residuals) %>% filter(Team == 'Maryland')
```

The Minnesota game that season was pretty wild - Maryland outgained the Gophers by **224 yards**, which our model predicts should have led to a **23-point** victory. What the model suggests should have been an easy win was anything but. But Minnesota isn't as hard done as Arizona was in the list above: they should have lost by 48, but ended up losing by 63.
